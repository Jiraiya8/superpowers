...
## 1. Superpowers Overall Architecture
...
graph TB
    subgraph CoreLayer["Core Layer"]
        ...
        Hooks["Hooks System"]
        Agents["Agent Config (AGENTS.md)"]
    end

    subgraph SkillLibrary["Skill Library"]
        ProcessSkills["Process Skills"]
        DebugSkills["Debugging Skills"]
        CollabSkills["Collaboration Skills"]
        MetaSkills["Meta Skills"]
    end

    subgraph PlatformAdaptation["Platform Adaptation Layer"]
        ClaudeCode["Claude Code"]
        Codex["Codex CLI/App"]
        OpenCode["OpenCode"]
        Copilot["Copilot CLI"]
    ...
    SkillsSystem --> SkillLibrary
    Hooks --> SkillsSystem
    Agents --> SkillsSystem
    SkillLibrary --> PlatformAdapt["Platform Adaptation Tools"]
    PlatformAdapt --> PlatformAdaptation
```
...
```mermaid
graph TB
    Start["User initiates task"] --> SkillCheck{"Any skill applicable?"
    
    SkillCheck -->|"Yes (even 1% chance)"| InvokeSkill["Invoke Skill tool"]
    SkillCheck -->|"Definitely not"| DirectResponse["Direct response"]
    
    InvokeSkill --> Announce["Announce: Using [skill] for [purpose]"]
    Announce --> HasChecklist{"Skill has checklist?"
    
    HasChecklist -->|"Yes"| CreateTodo["Create TodoWrite items"]
    HasChecklist -->|"No"| FollowSkill["Follow skill instructions"]
    
    CreateTodo --> FollowSkill
    
    FollowSkill --> Brainstorm{"Creative work needed?"
    
    Brainstorm -->|"Yes"| BrainstormingSkill["brainstorming skill"]
    Brainstorm -->|"No, has spec"| GitWorktrees["using-git-worktrees skill"]
    
    BrainstormingSkill --> DesignApproved{"Design approved?"
    DesignApproved -->|"No"| ReviseDesign["Revise design"]
    ReviseDesign --> BrainstormingSkill
    
    DesignApproved -->|"Yes"| GitWorktrees
    GitWorktrees --> WritingPlans["writing-plans skill"]
    
    WritingPlans --> ChooseExecution{"Choose execution method?"
    ChooseExecution -->|"Subagent-driven (recommended)"| SubagentDriven["subagent-driven-development"]
    ChooseExecution -->|"Inline execution"| ExecutingPlans["executing-plans skill"]
    
    SubagentDriven --> TDD["test-driven-development skill"]
    ExecutingPlans --> TDD
    
    TDD --> RequestReview["requesting-code-review skill"]
    RequestReview --> MoreTasks{"More tasks?"
    
    MoreTasks -->|"Yes"| SubagentDriven
    MoreTasks -->|"No"| FinishingBranch["finishing-a-development-branch skill"]
    
    FinishingBranch --> End["Complete"]
```
...
```mermaid
graph TB
    UserMessage["User message received"] --> PlanModeCheck{"About to EnterPlanMode?"
    
    PlanModeCheck -->|"Yes"| BrainstormedCheck{"Already brainstormed?"
    PlanModeCheck -->|"No"| MightApply{"Might any skill apply?"
    
    BrainstormedCheck -->|"No"| InvokeBrainstorm["Invoke brainstorming skill"]
    BrainstormedCheck -->|"Yes"| MightApply
    
    InvokeBrainstorm --> MightApply
    
    MightApply -->|"Yes, even 1%"| InvokeSkillTool["Invoke Skill tool"]
    MightApply -->|"Definitely not"| Respond["Respond (including clarifications)"
    
    InvokeSkillTool --> AnnounceUse["Announce: Using [skill] to [purpose]"]
    AnnounceUse --> ChecklistCheck{"Has checklist?"
    
    ChecklistCheck -->|"Yes"| CreateTodoItem["Create TodoWrite per item"]
    ChecklistCheck -->|"No"| FollowDirectly["Follow skill directly"]
    
    CreateTodoItem --> FollowDirectly
    FollowDirectly --> Respond
```

## 4. Brainstorming Skill Flow

```mermaid
graph TB
    ExploreContext["Explore project context"] --> VisualQ{"Visual questions ahead?"
    
    VisualQ -->|"Yes"| OfferVisual["Offer Visual Companion (own message)"]
    VisualQ -->|"No"| AskQuestions["Ask clarifying questions one at a time"]
    
    OfferVisual --> AskQuestions
    
    AskQuestions --> ProposeApproaches["Propose 2-3 approaches"]
    ProposeApproaches --> PresentDesign["Present design in sections"]
    
    PresentDesign --> UserApprove{"User approves design?"
    
    UserApprove -->|"No, revise"| PresentDesign
    UserApprove -->|"Yes"| WriteDesignDoc["Write design document"]
    
    WriteDesignDoc --> SpecSelfReview["Spec self-review"]
    SpecSelfReview --> UserReviewSpec{"User reviews spec?"
    
    UserReviewSpec -->|"Changes requested"| WriteDesignDoc
    UserReviewSpec -->|"Approved"| InvokeWritingPlans["Invoke writing-plans skill"]
    
    InvokeWritingPlans --> End["Enter planning phase"]
```

## 5. Git Worktrees Skill Flow

```mermaid
graph TB
    CheckExisting["Check existing directories"] --> FoundWorktrees{"Found .worktrees or worktrees?"
    
    FoundWorktrees -->|"Yes"| UseExisting["Use existing directory"]
    FoundWorktrees -->|"No"| CheckClaudeMd["Check CLAUDE.md config"]
    
    CheckClaudeMd --> HasPreference{"Has preference?"
    HasPreference -->|"Yes"| UsePreference["Use configured path"]
    HasPreference -->|"No"| AskUser["Ask user for choice"]
    
    AskUser --> UserChoice["User choice"]
    UseExisting --> VerifyIgnore["Verify directory is ignored"]
    UsePreference --> VerifyIgnore
    UserChoice --> VerifyIgnore
    
    VerifyIgnore --> IsIgnored{"git check-ignore confirms?"
    
    IsIgnored -->|"No"| AddGitignore["Add to .gitignore + commit"]
    IsIgnored -->|"Yes"| CreateWorktree["Create worktree"]
    
    AddGitignore --> CreateWorktree
    
    CreateWorktree --> ProjectSetup["Auto-detect and run project setup"]
    ProjectSetup --> VerifyTests["Verify clean test baseline"]
    
    VerifyTests --> TestsPass{"Tests pass?"
    TestsPass -->|"Yes"| ReportReady["Report ready"]
    TestsPass -->|"No"| ReportFail["Report failures + ask to proceed"]
```

## 6. Writing Plans Skill Flow

```mermaid
graph TB
    Announce["Announce using writing-plans skill"] --> ScopeCheck["Check spec scope"]
    
    ScopeCheck --> TooLarge{"Spans multiple independent subsystems?"
    TooLarge -->|"Yes"| SuggestSplit["Suggest breaking into multiple plans"]
    TooLarge -->|"No"| FileStructure["Map file structure"]
    
    SuggestSplit --> UserDecision{"User agrees to split?"
    UserDecision -->|"Yes"| CreateFirstPlan["Create first sub-plan"]
    UserDecision -->|"No"| FileStructure
    
    FileStructure --> DefineTasks["Define tasks (2-5 min per step)"]
    DefineTasks --> WriteHeader["Write plan document header"]
    
    WriteHeader --> TaskStructure["Write task structure"]
    TaskStructure --> NoPlaceholders["Ensure no placeholders"]
    
    NoPlaceholders --> SelfReview["Self-review plan"]
    SelfReview --> SpecCoverage{"Spec coverage complete?"
    
    SpecCoverage -->|"Missing"| AddMissingTask["Add missing tasks"]
    SpecCoverage -->|"Complete"| PlaceholderScan["Placeholder scan"]
    
    AddMissingTask --> SelfReview
    PlaceholderScan --> TypeConsistency["Type consistency check"]
    
    TypeConsistency --> SavePlan["Save plan document"]
    SavePlan --> OfferExecution["Offer execution options"]
    
    OfferExecution --> SubagentChoice["Subagent-driven (recommended)"]
    OfferExecution --> InlineChoice["Inline execution"]
    
    SubagentChoice --> InvokeSubagent["Invoke subagent-driven-development"]
    InlineChoice --> InvokeExecuting["Invoke executing-plans"]
```

## 7. Subagent-driven Development Skill Flow

```mermaid
graph TB
    ReadPlan["Read plan, extract all tasks"] --> CreateTodo["Create TodoWrite"]
    
    CreateTodo --> MoreTasks{"More tasks?"
    
    MoreTasks -->|"Yes"| DispatchImplementer["Dispatch implementer subagent"]
    MoreTasks -->|"No"| FinalReview["Dispatch final code reviewer"]
    
    DispatchImplementer --> HasQuestions{"Subagent has questions?"
    
    HasQuestions -->|"Yes"| AnswerQuestions["Answer questions, provide context"]
    HasQuestions -->|"No"| ImplementCommit["Subagent implements, tests, commits, self-reviews"]
    
    AnswerQuestions --> DispatchImplementer
    ImplementCommit --> DispatchSpecReviewer["Dispatch spec reviewer subagent"]
    
    DispatchSpecReviewer --> SpecCompliant{"Spec compliant?"
    
    SpecCompliant -->|"No"| FixSpecGaps["Implementer fixes spec gaps"]
    SpecCompliant -->|"Yes"| DispatchQualityReviewer["Dispatch code quality reviewer"]
    
    FixSpecGaps --> DispatchSpecReviewer
    DispatchQualityReviewer --> QualityApproved{"Quality approved?"
    
    QualityApproved -->|"No"| FixQualityIssues["Implementer fixes quality issues"]
    QualityApproved -->|"Yes"| MarkComplete["Mark task complete"]
    
    FixQualityIssues --> DispatchQualityReviewer
    MarkComplete --> MoreTasks
    
    FinalReview --> InvokeFinishing["Invoke finishing-a-development-branch"]
```

## 8. TDD (Test-Driven Development) Cycle

```mermaid
graph LR
    subgraph RED["RED Phase"]
        WriteFailTest["Write failing test"]
        VerifyFail["Verify test fails"]
    end
    
    subgraph GREEN["GREEN Phase"]
        MinimalCode["Write minimal code"]
        VerifyPass["Verify test passes"]
    end
    ...
    WriteFailTest --> VerifyFail
    VerifyFail -->|"Correct failure"| MinimalCode
    VerifyFail -->|"Wrong failure"| WriteFailTest
    
    MinimalCode --> VerifyPass
    VerifyPass -->|"Pass"| Cleanup
    VerifyPass -->|"Fail"| MinimalCode
    
    Cleanup --> StayGreen
    StayGreen --> VerifyPass
    VerifyPass -->|"All pass"| NextTest["Next test"]
    
    NextTest --> WriteFailTest
```

## 9. Systematic Debugging Four Phases

```mermaid
graph TB
    subgraph Phase1["Phase 1: Root Cause Investigation"]
        ReadErrors["Read error messages carefully"]
        Reproduce["Reproduce consistently"]
        CheckChanges["Check recent changes"]
        GatherEvidence["Gather evidence in multi-component systems"]
        TraceFlow["Trace data flow"]
    end
    
    subgraph Phase2["Phase 2: Pattern Analysis"]
        FindWorking["Find working examples"]
        CompareRef["Compare against references"]
        IdentifyDiff["Identify differences"]
        UnderstandDep["Understand dependencies"]
    end
    
    subgraph Phase3["Phase 3: Hypothesis Testing"]
        FormHypothesis["Form single hypothesis"]
        TestMinimally["Test minimally"]
        VerifyResult["Verify result"]
        DontKnow["Ask when you don't know"]
    end
    
    subgraph Phase4["Phase 4: Implementation"]
        CreateTest["Create failing test"]
        SingleFix["Implement single fix"]
        VerifyFix["Verify fix"]
        FixFailed{"Fix failed?"
        QuestionArch["Question architecture"]
    end
    
    ReadErrors --> Reproduce --> CheckChanges --> GatherEvidence --> TraceFlow
    TraceFlow --> Phase2Start["Enter Phase 2"]
    
    FindWorking --> CompareRef --> IdentifyDiff --> UnderstandDep
    UnderstandDep --> Phase3Start["Enter Phase 3"]
    
    FormHypothesis --> TestMinimally --> VerifyResult --> DontKnow
    VerifyResult -->|"Success"| Phase4Start["Enter Phase 4"]
    VerifyResult -->|"Failure"| FormHypothesis
    
    CreateTest --> SingleFix --> VerifyFix --> FixFailed
    FixFailed -->|"Success"| DebugDone["Debug complete"]
    FixFailed -->|"Failure (<3 times)"| Phase1Return["Return to Phase 1"]
    FixFailed -->|"Failure (≥3 times)"| QuestionArch
    
    QuestionArch --> DiscussPartner["Discuss architecture with user"]
```

## 10. Verification Before Completion Flow

```mermaid
graph TB
    AboutToClaim["About to claim completion"] --> IdentifyCommand["Identify verification command"]
    
    IdentifyCommand --> RunFull["Run FULL command (fresh, complete)"]
    RunFull --> ReadOutput["Read full output + check exit code"]
    
    ReadOutput --> VerifyClaim{"Output confirms claim?"
    
    VerifyClaim -->|"No"| StateActual["State actual status with evidence"]
    VerifyClaim -->|"Yes"| StateWithEvidence["State claim WITH evidence"]
    
    StateWithEvidence --> ThenClaim["Then make the claim"]
    
    AboutToClaim --> RedFlags{"Red flags present?"
    
    RedFlags -->|"Using 'should/probably/seems'"| RunFull
    RedFlags -->|"Expressing satisfaction before verification"| RunFull
    RedFlags -->|"No verification before commit"| RunFull
    RedFlags -->|"Trusting agent reports"| RunFull
```

## 11. Complete Session Lifecycle

```mermaid
graph TB
    SessionStart["Session Start"] --> LoadAgents["Load AGENTS.md"]
    
    LoadAgents --> UserInput["User Input"]
    
    UserInput --> SkillEvaluation["Skill Evaluation"]
    
    SkillEvaluation --> CreativeWork{"Creative work?"
    CreativeWork -->|"Yes"| BrainstormFlow["Brainstorming Flow"]
    CreativeWork -->|"No"| HasSpec{"Has spec?"
    
    BrainstormFlow --> DesignDone["Design Complete"]
    DesignDone --> HasSpec
    
    HasSpec -->|"Yes"| SetupWorktree["Setup Git Worktree"]
    HasSpec -->|"No"| MoreContext["Need more context"]
    
    MoreContext --> UserInput
    SetupWorktree --> CreatePlan["Create Implementation Plan"]
    
    CreatePlan --> ChooseMode{"Choose execution mode"
    
    ChooseMode -->|"Subagent"| SubagentLoop["Subagent Loop"]
    ChooseMode -->|"Inline"| InlineLoop["Inline Loop"]
    
    SubagentLoop --> TaskExecution["Task Execution (TDD)"]
    InlineLoop --> TaskExecution
    
    TaskExecution --> CodeReview["Code Review"]
    
    CodeReview --> AllComplete{"All tasks complete?"
    
    AllComplete -->|"No"| SubagentLoop
    AllComplete -->|"Yes"| FinalVerify["Final Verification"]
    
    FinalVerify --> ChooseFinish{"Choose finish option"
    
    ChooseFinish -->|"Merge"| MergeLocal["Local Merge"]
    ChooseFinish -->|"PR"| CreatePR["Create PR"]
    ChooseFinish -->|"Keep"| KeepBranch["Keep Branch"]
    ChooseFinish -->|"Discard"| DiscardWork["Discard Work"]
    
    MergeLocal --> Cleanup["Cleanup Worktree"]
    CreatePR --> Cleanup
    DiscardWork --> Cleanup
    KeepBranch --> SessionEnd["Session End"]
...
| Component | Purpose |
|-----------|---------|
| **using-superpowers** | Entry point - determines if/which skills apply |
...
| **writing-skills** | TDD for documentation creation |